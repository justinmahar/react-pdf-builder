import { Meta } from '@storybook/addon-docs';
import { DocsPDF } from '../../DocsPDF';

import { Signature } from '../../../../components/signature/Signature';
import { Paragraph } from '../../../../components/typography/Paragraph';

<Meta title="Documentation/Components/Signature" />

<h1>Signature</h1>

## Description

A Signature is simply a line with an X, creating an area for someone to sign a document.

## Usage

```jsx
import { Signature } from 'react-pdf-builder';
```

```jsx
<Signature x />
```

### Props

| Inherits all props from                                                                                      |
| ------------------------------------------------------------------------------------------------------------ |
| [Box](https://justinmahar.github.io/react-pdf-builder/?path=/docs/documentation-components-box--docs)        |
| [Div](https://justinmahar.github.io/react-pdf-builder/?path=/docs/documentation-components-basics-div--docs) |
| [View](https://react-pdf.org/components#view)                                                                |

And supports the following props:

| Prop     | Type          | Description                                                                                                                                                                  |
| -------- | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| lineSize | `number`      | Optional. Width of the line in points.                                                                                                                                                                   |
| x        | `boolean`     | Optional. When `true`, an `X` is shown above the line indicating where to sign.                                                                                                                                                                    |
| xProps   | `TextProps`   | Optional. Props for the `X` shown above the line.                                                                                                                                                                   |
| xValue   | `string`      | Optional. String value of the `X` shown above the line. Defaults to `"X"`.                                                                                                                                                                   |
| swatch   | `SwatchColor` | Optional. One of the [swatch color names](https://justinmahar.github.io/react-pdf-builder/?path=/docs/documentation-themes--docs#swatch-colors) from the theme, as a string. |

## Signature Example

<DocsPDF>
  <Signature className="mb-1" x />
  <Paragraph>Sign here</Paragraph>
</DocsPDF>

```jsx
import * as React from 'react';
import { Signature, Paragraph, ThemedPage } from 'react-pdf-builder';
import { PDFViewer, Document } from '@react-pdf/renderer';

export function Example() {
  return (
    <PDFViewer style={{ width: 420, height: 600 }}>
      <Document>
        <ThemedPage size="LETTER">
          <Signature className="mb-1" x />
          <Paragraph>Sign here</Paragraph>
        </ThemedPage>
      </Document>
    </PDFViewer>
  );
}
```
